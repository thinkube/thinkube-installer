...
# A file that says it is generated is rewritten by its generator, and a file
# listed in .licenseignore belongs to someone else. Neither takes a notice.
GENERATED_MARKERS = [
    'NEVER EDIT MANUALLY',
    'DO NOT EDIT',
...
def ignored_paths(repo_root: Path) -> List[str]:
    """The path patterns listed in .licenseignore, one per line.

    A repository uses it for files it did not write: vendored scripts,
    upstream configuration, dashboards derived from another project.
    """
    ignore = repo_root / IGNORE_FILE
...
def is_generated(content: str) -> bool:
    """Whether the first 20 lines say a generator owns this file."""
    head = '\n'.join(content.split('\n')[:20]).lower()
...
def main() -> None:
    ...
    ignore_patterns = ignored_paths(repo_root)
    ...
    for filepath in tracked_files(repo_root):
        ...
        if SKIP_PATH_PARTS & set(relative.parts[:-1]):
            stats['generated'] += 1
            continue
        if any(relative.match(pattern) or str(relative).startswith(pattern.rstrip('/') + '/')
    ...
    print(f"  generated or marked:   {stats['generated']}")
    if stats['ignored']:
        print(f"  in {IGNORE_FILE}:       {stats['ignored']}")
    if stats['binary']:
