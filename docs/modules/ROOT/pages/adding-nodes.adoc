...
* Each new node meets the xref:node-setup.adoc[prerequisites] (Ubuntu 24.04, SSH, internet access)
...
[arabic]
. Installs and configures the overlay provider (ZeroTier or Tailscale) on the new node
. Distributes SSH keys for inter-node communication
. Updates the Ansible inventory
. Joins each node to the Kubernetes cluster
. Cordons the new nodes (prevents scheduling)
. Configures DNS resolution for internal domains
. Deploys the GPU operator (if GPUs detected)
. Configures GPU time-slicing profiles
...
[arabic]
. *Mirror public images* — pulls images from Docker Hub, GCR, Quay.io and pushes them to your Harbor registry with per-architecture support
. *Build base images* — builds platform images (Python, Node.js, CI tools, AI inference) natively on each architecture's node
. *Build Jupyter image* — builds the JupyterLab image with GPU support
...
[arabic]
. Checks Harbor for each image's per-arch tag (e.g., `+python-base:3.12-slim-amd64+`)
. Skips images that already exist for that architecture
. Builds missing images *natively* on a node of the matching architecture — no QEMU emulation
. Creates multi-arch manifest lists combining both architectures
. Pushes the manifest to Harbor so `+podman pull+` or `+containerd+` automatically selects the right architecture
...
* link:/architecture/multi-architecture/[Multi-Architecture Support] — How native per-arch builds work
* link:/thinkube-control/registry/[Container Registry] — Manage images in Harbor
* link:/learn/your-first-deploy/[Your First Deploy] — Deploy an application on your expanded cluster
